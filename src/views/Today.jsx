...
export default function Today() {
  ...
  const handleExportShare = async () => {
    try {
      ...
      const backupFile = new File([blob], fileName, {
  ...
  const scrollToQuickCapture = () => {
    ...
    if (quickCaptureSection) {
      quickCaptureSection.scrollIntoView({
        behavior: "smooth",
        block: "start",
      });
    }
  ...
  const showFocusSection =
    filter === "today" || filter === "all" || filter === "overdue";
    ...
    <div className="app-container">
      ...
      <main className="main-content">
        ...
        {!showFocusSection && filteredTasks.length === 0 && (
          <div className="empty-state">
            <p>
              {t("today.noTasksForFilter", {
                filter: t(`today.filters.${filter}`),
              })}
            </p>
          </div>
        )}
      ...
      {exportMenuOpen && (
        <>
          ...
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label={t("backup.exportDialogTitle")}
          >
