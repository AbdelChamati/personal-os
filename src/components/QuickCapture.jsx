...
function parseNaturalInput(input) {
  ...
  const minutesMatch = remaining.match(/\b(\d{1,3})\s*(m|min|mins|minute|minutes)\b/i);
  if (minutesMatch) {
  ...
  const timeMatch = remaining.match(/\b(?:at\s*)?(\d{1,2})(?::(\d{2}))?\s*(am|pm)?\b/i);
  if (timeMatch) {
...
export default function QuickCapture({ onTaskCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const resolvedCategory = category === 'Personal' && parsed.category ? parsed.category : category;
      const resolvedPriority = priority === 'medium' && parsed.priority ? parsed.priority : priority;
      const resolvedMinutes = estimatedMinutes || parsed.estimatedMinutes || null;
      const resolvedDueAt = dueDate ? new Date(dueDate).toISOString() : parsed.dueAt;

      const taskData = {
        title: resolvedTitle,
        category: resolvedCategory,
        priority: resolvedPriority,
        estimated_minutes: resolvedMinutes ? parseInt(resolvedMinutes, 10) : null,
      ...
      const hints = [];
      if (parsed.priority) hints.push(t('quickCapture.hintPriority', { value: t(`priorities.${parsed.priority}`) }));
      if (parsed.estimatedMinutes) hints.push(t('quickCapture.hintMinutes', { value: parsed.estimatedMinutes }));
      if (parsed.category) hints.push(t(`categories.${parsed.category}`));
      if (parsed.dueAt) hints.push(t('quickCapture.dueDateTime'));
      setParseHint(hints.length ? t('quickCapture.autoDetected', { details: hints.join(' • ') }) : '');
